...
def truncated_normal_(my_tensor, mean=0, std=0.1):
    """
    :: 功能: 初始化一个正态分布取值、有范围限定（-2，2）的 W 矩阵和 bias 矩阵
    :: 输入: 一个 Tensor.Parameter 类型的尚未初始化 weight 矩阵和 bias 矩阵
    :: 输出: 一个 Tensor.Parameter 类型的初始化后(有取值范围限定)的 weight 矩阵和 bias 矩阵
    """
    size = my_tensor.shape
    tmp = my_tensor.new_empty(size + (4,)).normal_()
    valid = (tmp < 2) & (tmp > -2)
    ind = valid.max(-1, keepdim=True)[1]    # valid 每一行的最大值的 index 组成的 Tensor 矩阵
    my_tensor.data.copy_(tmp.gather(-1, ind).squeeze(-1))
    my_tensor.data.mul_(std).add_(mean)
    return my_tensor
